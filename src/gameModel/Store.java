...
public class Store {
	...
	private void buildStoreItems() {
		...
		if (chance < 75)
			this.items.add(new StoreItem("Something", 1));
		// Half Stores (50%) will Sell ???
		if (chance < 50)
			this.items.add(new StoreItem("Something Else", 1));
		// Few Stores (25%) will Sell ???
		if (chance < 25)
			this.items.add(new StoreItem("Rare Item", 1));
		// Even Less Stores (10%) will Sell ???
		if (chance < 10)
			this.items.add(new StoreItem("Very Rare Item", 1));
		// One in 100 Stores will Sell ???
		if (chance < 2)
			this.items.add(new StoreItem("Extreamly Rare Item", 1));
	}
